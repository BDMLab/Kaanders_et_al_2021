...
def simulate_trial_drift(parameters, values, gaze, boundary=1, error_weight=0.05, error_range=(0, 5000),extra_s = 0):
    ...
    while (Evi0[time] < boundary) and (Evi1[time] < boundary):
            ...
            #lam = (boundary / s)**2
            Evi0_new = Evi0[time] +  drifts[0] +  np.random.normal(0,s + s0)
            Evi1_new = Evi1[time] +  drifts[1] +  np.random.normal(0,s + s1)
