...
def plot_fit(data, predictions, color_data = '#4F6A9A',label1 = 'Human',label2 = ' Simulations', Inverse = False, GazePlot = True):
...
def add_left_minus_mean_others(df):
    ...
    values = df[value_cols].values
    left_minus_mean_others = values[:, 0] - np.mean(values[:, 1])
...
def add_left_gaze_advantage(df):
    ...
    left_gaze_advantage_raw = gaze[:, 0] - np.mean(gaze[:, 1])
...
def plot_pleft_by_left_gaze_advantage(data, predictions=None, ax=None, n_bins=8, xlabel_skip=2, color1 = '#4F6A9A'):
    ...
    for i, dataframe in enumerate(dataframes):
        ...
        if len(df['left_gaze_advantage_bin'].unique())<2: # check if we have only one bin
